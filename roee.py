...
class TestServerClientInteraction(unittest.TestCase):
    ...
    def test1(self):
        ...
        self.assertIn('HTTP/1.1 200 OK', response,
                      '[TEST 1] Header was not printed correctly')
        self.assertTrue(os.path.exists(filename),
                        "[TEST 1] /index.html file not created by the client")
        ...
        self.assertEqual(expected_contents, retrieved_contents,
                         "[TEST 1] /index.html file content does not match the expected content")
        os.remove(filename)

    def test2(self):
        """Test retrieving a non-existing file from the server."""
        response = self.send_request_and_receive_response(
            '/non_existent_file.html')
        self.assertIn('HTTP/1.1 404 Not Found', response,
                      "[TEST 2] Non-existent file did not return correctly")

    def test3(self):
        """Test a redirect response from the server."""
        response = self.send_request_and_receive_response('/redirect')
        self.assertIn('HTTP/1.1 301 Moved Permanently', response,
                      "[TEST 3] Redirect response did not return correctly")
        response = self.client_process.stdout.readline()
        self.assertIn('HTTP/1.1 200 OK', response,
                      "[TEST 3] Redirected file did not return correctly")
        filename = 'result.html'
        self.assertTrue(os.path.exists(filename),
                        "[TEST 3] /result.html file not created by the client")
        with open(filename, 'r') as file:
            retrieved_contents = file.read()
        ...
        with open(expected_file_path, 'r') as file:
            expected_contents = file.read()
        self.assertEqual(expected_contents, retrieved_contents,
                         "[TEST 3] /result.html file content does not match the expected content")
        os.remove(filename)
    ...
    def test4(self):
        ...
        list_files = ['/index.html', 'index.html', '/a/b/ref.html', '/c/footube.css',
                      'c/Footube.html', '/c/footube.js', '/result.html', '/', '']
        for path in list_files:
            response = self.send_request_and_receive_response(path)
            self.assertIn('HTTP/1.1 200 OK', response,
                          f"[TEST 4] did not return correctly for {path}")
            ...
            self.assertTrue(os.path.exists(filename), f"[TEST 4] {
                            path} file not created by the client")
            with open(filename, 'r') as file:
                retrieved_contents = file.read()
            ...
            with open(expected_file_path, 'r') as file:
                expected_contents = file.read()
            self.assertEqual(expected_contents, retrieved_contents, f"[TEST 4] {
                             path} file content does not match the expected content")
            # Clean up by removing the file after the test
            os.remove(filename)

    def test5(self):
        """Test with images"""
        list_files = ['/a/1.jpg', 'a/2.jpg', '/a/3.jpg', '/a/4.jpg', 'a/5.jpg', '/a/6.jpg', '/a/b/1.jpg', 'a/b/2.jpg', '/a/b/3.jpg', '/a/b/4.jpg',
                      '/a/b/5.jpg', '/a/b/6.jpg', '/c/img/1.jpg', '/c/img/2.jpg', '/c/img/3.jpg', '/c/img/4.jpg', '/c/img/5.jpg', '/c/img/6.jpg', '/favicon.ico']
        for path in list_files:
            response = self.send_request_and_receive_response(path)
            self.assertIn('HTTP/1.1 200 OK', response)
            # Ensure the file was created by the client
            filename = path.split('/')[-1]
            self.assertTrue(os.path.exists(filename), f"[TEST 5] {
                            path} file not created by the client")
            ...
            with open(expected_file_path, 'rb') as file:
                expected_contents = file.read()
            self.assertEqual(expected_contents, retrieved_contents, f"[TEST 5] {
                             path} file content does not match the expected content")
            # Clean up by removing the file after the test
            os.remove(filename)
    ...
    def test6(self):
        ...
        bad_req = ['Roee', '//', '///', '/////', 'bad.html', '/a', '/a/b', '/a/b/',
                   '/a/b/1', '/a/b/1.', '/a/b/1.j', '/a/b/1.jp', 'index.html/']
        for path in bad_req:
            response = self.send_request_and_receive_response(path)
            self.assertIn('HTTP/1.1 404 Not Found', response)
    ...
    def test8(self):
        ...
        list_files = ['/index.html', '/a/1.jpg', '/result.html',
                      '/a/b/1.jpg', 'a/b/ref.html', '/a/2.jpg', 'c/footube.css']
        for path in list_files:
            mode = 'rb' if path.endswith(
                ('.png', '.jpg', '.jpeg', 'ico')) else 'r'
            response = self.send_request_and_receive_response(path)
            self.assertIn('HTTP/1.1 200 OK', response)
            # Ensure the file was created by the client
            filename = path.split('/')[-1]
            self.assertTrue(os.path.exists(filename), f"[TEST 8] {
                            path} file not created by the client")
            ...
            with open(expected_file_path, mode) as file:
                expected_contents = file.read()
            self.assertEqual(expected_contents, retrieved_contents, f"[TEST 8] {
                             path} file content does not match the expected content")
            # Clean up by removing the file after the test
            os.remove(filename)
    ...
    def test9(self):
        ...
        self.assertTrue(os.path.exists(
            filename), "[TEST 9] /large_file.txt file not created by the client")
        with open(filename, 'r') as file:
            retrieved_contents = file.read()
        with open(large_file_path, 'r') as file:
            expected_contents = file.read()
        self.assertEqual(expected_contents, retrieved_contents,
                         "[TEST 9] /large_file.txt file content does not match the expected content")
        os.remove(filename)
    ...
    def test11(self):
        ...
        self.assertTrue(os.path.exists(filename), f"[TEST 11] {
                        text_file_path} file not created by the client")
        ...
        # Compare the bytes
        self.assertEqual(expected_contents, retrieved_contents, f"[TEST 11] {
                         text_file_path} file content does not match the expected content")
        ...
        self.assertTrue(os.path.exists(filename), f"[TEST 11] {
                        img_file_path} file not created by the client")
        with open(filename, 'rb') as file:
            retrieved_contents = file.read()
        ...
        with open(expected_file_path, 'rb') as file:
            expected_contents = file.read()
        self.assertEqual(expected_contents, retrieved_contents, f"[TEST 11] {
                         img_file_path} file content does not match the expected content")
        os.remove('index.html')
